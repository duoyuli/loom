...
source: "https://www.bestblogs.dev/article/c725aea4?entry=newsletter_page"
author:
  - "[[Latent.Space]]"
published: 2026-04-07
created: 2026-04-19
description: "OpenAI 的 Ryan Lopopolo 介绍了“Harness Engineering”，这是一种激进的工作流，通过每天消耗 10 亿 Token，构建了一个拥有 100 万行代码且 0% 人工编写的代码库。"
tags:
  - "clippings"
---
93
...
OpenAI 的 Ryan Lopopolo 介绍了“Harness Engineering”，这是一种激进的工作流，通过每天消耗 10 亿 Token，构建了一个拥有 100 万行代码且 0% 人工编写的代码库。

⭐ 推荐理由

OpenAI 内部实验揭示了「Harness Engineering」的巨大潜力：在零人工代码介入下完成百万行级别的产品开发。核心要点在于通过构建强力的支架系统与可观测性链路，消除人工审核瓶颈，使智能体成为真正的队友。文章重点介绍了 Symphony 编排系统如何利用代码驱动的规范（Spec）处理复杂的 PR 周期，并提出软件架构应向「智能体可读性」转型的核心观点。推荐给追求极致 AI 自动化效率的开发者与架构师。

[Latent Space](https://www.bestblogs.dev/articles?sourceid=fc479e "查看该来源的更多文章")

04-0716148 字 (约 65 分钟)

[查看原文 →](https://www.latent.space/p/harness-eng)

### 摘要

本文探讨了由 Ryan Lopopolo 领导的 OpenAI Frontier 团队进行的一项开创性实验，该实验将软件开发范式从“上下文工程（Context Engineering）”转向了“Harness Engineering”。在五个月的时间里，团队构建了一个拥有超过 100 万行代码和数千个 PR 的内部产品，且在合并前实现了零人工编写或审查代码。其核心理念是将人类视为瓶颈，将 AI 智能体视为主要开发者。关键策略包括：保持严格的一分钟构建循环以确保智能体生产力，构建“智能体可读（agent-legible）”的可观测性堆栈，以及使用“Symphony”（一个内部编排层）来协调自主智能体。文章认为，在“Token 亿万富翁”时代，软件的设计应优先考虑模型消费而非人类习惯，从而使代码变得越来越可抛弃且由规范驱动。

### 主要内容

- 1\. 从上下文工程（Context Engineering）向 Harness Engineering 的转变。
	工程师不再仅仅是优化提示词，而是必须构建整个环境（即脚手架/Harness），包括构建系统、可观测性和护栏，使智能体能够自主运行。
- 2\. 人类是 AI 原生开发中的主要瓶颈。
	由于 Token 成本低廉且智能体可并行化，人类的同步注意力成了最稀缺的资源。目标是将人工审查移至合并后，或通过自动化的系统级检查来消除人工审查。
- 3\. 软件必须为“智能体可读性”而设计。
	代码库和工具应优先考虑快速反馈循环（例如 < 1 分钟的构建）和结构化上下文（Markdown 追踪器、技能），以便模型能够轻松解析并采取行动。
- 4\. “幽灵库（Ghost Libraries）”与规范驱动开发的概念。
	利用高保真规范（PRD）让智能体能够自主复现复杂系统，将实现视为可抛弃且易于重新生成的内容。

*We’re proud to release this ahead of [Ryan’s keynote at AIE Europe](https://www.youtube.com/watch?v=O_IMsEg91g8). Hit the bell, get notified when it is live! Attendees: come prepped for [Ryan’s AMA with Vibhu after](https://www.ai.engineer/schedule).*

Move over, [context engineering](https://x.com/karpathy/status/1937902205765607626). Now it’s time for **Harness engineering** and the age of the [token billionaires](https://x.com/_dmca/status/2029810231325380725).

![](https://www.youtube.com/watch?v=CeOXx-XTYek)
[↗ 在新标签页打开嵌入内容](https://www.youtube-nocookie.com/embed/CeOXx-XTYek?rel=0&autoplay=0&showinfo=0&enablejsapi=0)

**Ryan Lopopolo** of OpenAI is leading that charge, recently publishing [a lengthy essay on Harness Eng](https://openai.com/index/harness-engineering/) that has become the talk of the town:

![](https://image.jido.dev/20260407173303_9553ccc5-17b8-4766-8921-2a1530470e97_1597x1360.png)

fuller discussion between Bret and Ryan

In it, Ryan peeled back the curtains on how the recently announced [OpenAI Frontier](https://openai.com/index/introducing-openai-frontier/) team have become OpenAI’s top Codex users, running a >1m LOC codebase with [0 human written](https://x.com/_lopopolo/status/2036153987674898611) code and, crucially [for the Dark Factory fans](https://www.latent.space/p/reviews-dead), no [human REVIEWED code before merge](https://x.com/_lopopolo/status/2037291250072932493). Ryan is admirably evangelical about this, calling it borderline “negligent” if you aren’t using >1B tokens a day (**[roughly $2-3k/day in token spend](https://x.com/swyx/status/2030080965020897753)** based on market rates and caching assumptions):

![](https://image.jido.dev/20260407173303_66ee57d3-8573-4a51-8f9f-c11437d1c4f9_1188x708.png)

[search it](https://x.com/search?q=from%3A_lopopolo%2050&src=typed_query&f=top)
...
The result was [Symphony](https://github.com/openai/symphony), “a ghost library” and reference Elixir implementation ([by Alex Kotliarskyi](https://x.com/alex_frantic/status/2030400081636290748)) that sets up a massive system of Codex agents all extensively prompted with the specificity of a proper PRD spec, but without full implementation:

![](https://image.jido.dev/20260407173303_ffccec93-8328-437b-9974-c2df897604cc_1508x1592.png)

The future starts taking shape as one where coding agents stop being copilots and start becoming real teammates anyone can use and [Codex](https://openai.com/codex/) is doubling down on that mission with their Superbowl messaging of **“you [can just build things](https://x.com/swyx/status/2023475672157696079) ”.**

Across Codex, internal observability stacks, and [the multi-agent orchestration system his team calls](https://github.com/openai/symphony) **[Symphony](https://github.com/openai/symphony)**, Ryan has been pushing what happens when you optimize an entire codebase, workflow, and organization around agent legibility instead of human habit.
...
- **X:** [https://x.com/\_lopopolo](https://x.com/_lopopolo)
- **Linkedin:** [https://www.linkedin.com/in/ryanlopopolo/](https://www.linkedin.com/in/ryanlopopolo/)
- **Website:** [https://hyperbo.la/contact/](https://hyperbo.la/contact/)
