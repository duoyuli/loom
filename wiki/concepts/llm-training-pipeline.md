...
- 推理系统则是部署反馈中的另一条硬约束：`prefill / decode`、KV cache、Prompt Cache、量化、continuous batching 和 speculative decoding 会决定模型能以什么成本、延迟和并发形态被交付给用户。
...
- 用户感知到的“更便宜”也不一定来自模型本身更小；可能来自 Prompt Cache 命中、稳定前缀设计和供应商缓存定价。
...
- [LLM 推理系统](llm-inference-systems.md) 负责承接部署和 serving 侧细节，尤其是缓存、批处理、延迟和成本指标。
