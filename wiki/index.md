...
- 当前 `sources/` 已覆盖 Agentic Engineering、Agent 工程、Memory、Harness、Context、训练栈和 Transformer 前史等主题；Harness 线索也继续从高自动化编排与 `agent-legible` 仓库，扩展到企业工程里的 `spec / handoff / checkpoint / evidence` 这组协作协议。查找具体来源时，优先从来源索引反查，而不是猜文件名。
